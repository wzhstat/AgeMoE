...
if __name__ == "__main__":
    ...
    print('********calculating condition_exp_mean**************')
    condition_exp_mean_pre = {}

    for cond, df in tqdm(all_pre_df.groupby('condition')):
        drug_mean = df.mean(numeric_only=True)
        condition_exp_mean_pre[cond] = drug_mean

    condition_exp_mean_pre_df = pd.DataFrame.from_dict(condition_exp_mean_pre, orient='index')



    condition_exp_mean_t = {}

    for cond, df in tqdm(all_true_df.groupby('condition')):
        drug_mean = df.mean(numeric_only=True)
        condition_exp_mean_t[cond] = drug_mean
    ...
    results_dict['condition_pearson'] = pearson_mean(condition_exp_mean_pre_df.to_numpy(), condition_exp_mean_t_df.to_numpy())
